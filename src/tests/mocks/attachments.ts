...
export const getAttachmentById = jest.fn((id: number) =>
  Promise.resolve({ id: id, name: 'attachment', url: 'https://attachment.com/4vd1o' })
);

export const assignToGroupLesson = jest.fn(
  (attachmentId: number, lessonId: number, groupId: number) => Promise.resolve()
);

export const getNonexistentAttachmentById = jest.fn((id: number) => Promise.resolve(null));

export const getAttachmentsArray = jest.fn((lessonId: number, groupId: number) =>
  Promise.resolve([
...
export const deleteGroupLessonAttachment = jest.fn(
  (attachmentId: number, lessonId: number, groupId: number) => Promise.resolve()
);
